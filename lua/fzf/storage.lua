...
local sessions_dir_ready = false
local function ensure_sessions_dir()
  if not sessions_dir_ready then
    vim.fn.mkdir(M.sessions_dir, "p")
    sessions_dir_ready = true
  end
end

M.ensure_sessions_dir = ensure_sessions_dir

M.harpoon_file = vim.fn.stdpath("data") .. "/harpoon.json"

function M.harpoon_load()
  local f = io.open(M.harpoon_file, "r")
...
function M.harpoon_save(data)
  local f = io.open(M.harpoon_file, "w")
